...
class Game(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.text       = self.font.render("Press SPACE to restart", False, (0, 0, 0))

        self.background = Screen_Obj("pics/bg.png", C.SCREEN_WIDTH, 
        C.SCREEN_HEIGHT, (C.SCREEN_WIDTH/2, C.SCREEN_HEIGHT/2))
        self.restart    = Screen_Obj("pics/bg.png", C.SCREEN_WIDTH, 
        C.SCREEN_HEIGHT, (C.SCREEN_WIDTH/2, C.SCREEN_HEIGHT/2))
    ...
    def update(self):
        ...
        while running:
            ...
            if self.player2 not in self.group or self.player1 not in self.group:
                self.group.empty()
                self.group.add(self.restart, self.restart_t)
